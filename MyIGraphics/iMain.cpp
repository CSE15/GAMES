...
void start_scr(){
    ...
    iText(150,450, "THE SNAKE LOSES IT'S LIFE IF IT HITS THE BOUNDERY OR THE TAIL!",GLUT_BITMAP_TIMES_ROMAN_24);
    iSetColor(250, 0, 150);
...
void block(){
    ...
    if(snake[0][0]>=700-r && snake[0][0]<=800+r && snake[0][1]>=300-r && snake[0][1]<=350+r){
        gameover();
        PlaySound("F:\SOUNDS\\jump19.wav",NULL,SND_SYNC);
    }
...
void reset_pos()
{
    ...
    snake[0][1]=300;
    movex=10; movey=0;
    PlaySound("F:\SOUNDS\\Powerup4.wav",NULL,SND_SYNC);
}
...
void gameloop(){
    ...
    for(i=0;i<snake_len;i++){
        iSetColor(255,20+i*10,0);
        if(i==0){
            iFilledCircle(snake[i][0],snake[i][1],r*1.3);
            continue;
        }
...
/*    if(highScore == NULL)
    {
        highScore = realloc(highScore,2*N*sizeof(hScore));
    }*/
    ...
    for(k=1; k<=10; k++)
    {
        printf("%2d)  %s %d\n",k,names_for_files[k],scores_for_files[k]);
...
void iSpecialKeyboard(unsigned char key) {
    if(gameover_var==1 && current_key!=prev_key){
        ...
        else if (key == GLUT_KEY_UP && movey!=-(2*r)) {
            movey=(2*r);
            movex=0;
